...
\`\`\`javascript
// A1-car-user-app/__tests__/TirePressureDisplay.test.js
describe('TirePressureDisplay', () => {
  ...
  it('should update in real-time', async () => {
    const { getByText, rerender } = render(<TirePressureDisplay pressures={{ FL: 2.2 }} />);
    expect(getByText('FL: 2.2 bar')).toBeTruthy();
    
    rerender(<TirePressureDisplay pressures={{ FL: 1.8 }} />);
...
\`\`\`javascript
// A1-car-user-app/__tests__/TirePressureColor.test.js
describe('Tire Pressure Color Coding', () => {
  it('should show red for low pressure', () => {
...
\`\`\`javascript
// A1-car-user-app/__tests__/TirePressureAlert.test.js
describe('Tire Pressure Alerts', () => {
  it('should trigger alert when tire is low', () => {
  ...
  it('should dismiss alert when pressure normalizes', () => {
    const { rerender } = render(<CarDashboard pressures={{ FL: 1.6 }} />);
    rerender(<CarDashboard pressures={{ FL: 2.3 }} />);
...
\`\`\`javascript
// B1-web-server/tests/api.test.js
describe('GET /api/car/:licensePlate', () => {
  it('should include tire pressure in response', async () => {
    ...
    expect(response.body.tirePressure).toHaveProperty('FL');
    expect(response.body.tirePressure).toHaveProperty('FR');
    expect(response.body.tirePressure).toHaveProperty('RL');
    expect(response.body.tirePressure).toHaveProperty('RR');
  });
  
  it('should validate pressure range', async () => {
    const invalidData = { tirePressure: { FL: 5.0 } };
  ...
  it('should calculate low pressure alert', async () => {
    await db.collection('cars').updateOne(
      { licensePlate: 'ABC123' },
      { \\$set: { tirePressure: { FL: 1.7, FR: 2.2, RL: 2.1, RR: 2.2 } } }
    );
...
\`\`\`javascript
// B2-iot-gateway/tests/websocket.test.js
describe('WebSocket Broadcasting', () => {
  it('should broadcast tire pressure updates', (done) => {
    ...
    client.on('message', (data) => {
      ...
      expect(message.data).toHaveProperty('FL');
      done();
    });
    
    redis.publish('sensors:tire_pressure', JSON.stringify({
      licensePlate: 'ABC123',
  ...
  it('should handle multiple clients', async () => {
    ...
    redis.publish('sensors:tire_pressure', JSON.stringify({ test: true }));
...
\`\`\`javascript
// B3-realtime-database/tests/queries.test.js
describe('MongoDB Tire Pressure Operations', () => {
  it('should store tire pressure', async () => {
    const data = {
      licensePlate: 'ABC123',
      tirePressure: { FL: 2.2, FR: 2.3, RL: 2.1, RR: 2.2 },
      timestamp: new Date()
